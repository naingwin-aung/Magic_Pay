...
class PageController extends Controller
{
    ...
    public function toAccountVerify(Request $request)
    {
        if($request->to_phone) {
            ...
            if(is_null($to_account) || $to_account->phone === $user->phone) {
                return fail('ငွေလွှဲလိုသောဖုန်းနံပါတ် မရှိပါ။(မိမိဖုန်းနံပါတ် မဟုတ်ရပါ။)', null);
            }
            
            return success('success', ['name' => $to_account->name, 'phone' => $to_account->phone]);
        }

        return fail('ငွေလွှဲလိုသောဖုန်းနံပါတ် ထည့်ရန်လိုအပ်သည်။', null);
    }
    ...
    public function transferConfirm(TransferFormRequest $request)
    {
        ...
        if($request->hash_value !== $hash_value2) {
            return fail('မလုံခြူံသော အချက်အလက်များဖြစ်နေပါသည်။', null);
        }
        ...
        if($amount > $from_account->wallet->amount) {
            return fail('ငွေလွှဲလိုသောပမာဏ မလုံလောက်ပါ။', null);
        }
        
        if($to_account && $to_account->phone !== $from_account->phone) {
            return success('success', [
                'from_name' => $from_account->name,
                'form_phone' => $from_account->phone,
    ...
    {
        if(!$request->password) {
            return fail('လျိူ့၀ှက်နံပါတ် ထည့်ရန်လိုအပ်သည်။', null);
        }

        $from_account = Auth::user();
        if(!Hash::check($request->password, $from_account->password)) {
            return fail('လျိူ့၀ှက်နံပါတ် မှားယွင်းနေပါသည်။', null);
        }
        ...
        if($to_account && $from_account->phone !== $to_account->phone) {
